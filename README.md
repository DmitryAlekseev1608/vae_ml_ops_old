...
8. Если и это не помогло, то выполнить (указать имя USER):
    ...
    ```
9. Инициализация poetry (если появились ошибки или предупреждения, то прошу пересоздать виртуальную среду,
   я запускал установку на трех компьютерах и на одном выскочила проблема, которая при повторном создании             виртуальной среды исчезла):
    ```bash
    poetry install
    ```
11. Установка pre-commit:
    ```bash   
    pre-commit install
    ```
12. По ТЗ обещали, что сервер mlflow будет развернут на "http://128.0.1.1:8080".
13. Запуск исполнения кода:
    ```bash     
...
14. Требуемая при проверке по ТЗ операция выполняется:
    ```bash
    pre-commit run -a
    ```
15. Запуск тестового сервера ml-flow:
    ```bash
